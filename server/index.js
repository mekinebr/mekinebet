...
export default function App() {
  ...
  const sinaisFiltrados = useMemo(() => {
    return signals
      .filter((item) => {
        ...
        if (filtro === "TOP IA") return cat === "TOP IA" || normalizar(`${item.market} ${item.category} ${item.categoria}`).includes("top ia") || normalizar(`${item.market} ${item.category} ${item.categoria}`).includes("topia") || (item.confidence || 70) >= 82;
  ...
  const realLiveCount = signals.filter((s) => s.type === "live" && s.source === "api").length;
  ...
  const vipCount = signals.filter(isVip).length;
